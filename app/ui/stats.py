...
def calc_stats(state, markups: list[ECGMarkup]):
    ...
    locale = state["locale"]
    dpg.delete_item(STATS_WINDOW_TAG, children_only=True)
    ...
    with dpg.plot(tag=STATS_TAG, height=DISPLAY_PLOT_SIDE * scale, width=DISPLAY_PLOT_SIDE * 2 * scale, parent=STATS_WINDOW_TAG, no_mouse_pos=True):
        ...
        # dpg.add_text(, parent=STATS_WINDOW_TAG)
        ...
        dpg.add_plot_annotation(label=f"{tr(texts.COUNT, locale)}, {tr(texts.AVG, locale)}")
